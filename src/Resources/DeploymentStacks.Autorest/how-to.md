# How-To
This document describes how to develop for `Az.DeploymentStacks`.

## Building `Az.DeploymentStacks`
...
There are multiple scripts created for performing different actions for developing `Az.DeploymentStacks`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DeploymentStacks.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DeploymentStacks.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DeploymentStacks`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DeploymentStacks` into the session.
